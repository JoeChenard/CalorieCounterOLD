...
func _ready():
	...
	for id in global.trackingdic[global.date]['lunch'].keys():
		if not(id in global.menudic):
			...
			newitem._on_input_value_changed(global.trackingdic[global.date]['lunch'][id])
			newitem.get_child(1).get_line_edit().text = str(global.trackingdic[global.date]['lunch'][id])


func updateMeal(itemName, value):
#	print('tracking' + str(global.trackingdic))
	global.trackingdic[global.date]['lunch'][itemName] = value
